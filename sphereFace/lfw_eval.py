...
def cropping(src_img ):
    ...
    leading_horz = int((img_arr.shape[0] - 96)/2)
    trailing_horz = int(img_arr.shape[0] - leading_horz)
    leading_vert = int((img_arr.shape[1] - 112)/2)
    trailing_vert = int(img_arr.shape[1] - leading_vert)
    face_img = img_arr[leading_horz:trailing_horz, leading_vert:trailing_vert, :]
    face_img = np.asarray(face_img )[:, :, ::-1]
    return face_img
