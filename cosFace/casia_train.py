...
def experiment_name(opt):

    exp_name = "bn_checkpoint_m_5"
    return exp_name
...
parser.add_argument('--checkpoint_path', default='./bn_checkpoint_m_5/model_checkpoint')
parser.add_argument('--experiment', default='bn_checkpoint_m_5', help='the path to store sampled images and models')
...
if opt.tensorboard_logging:
    writer = SummaryWriter(os.path.join(opt.logdir, experiment_name(opt)))
...
def train(start_epoch, total_epoch, iteration, net, optimizer, checkpoint_path):
    for epoch in range(start_epoch, total_epoch):
        ...
        for i, dataBatch in enumerate(faceLoader ):
            ...
            if opt.tensorboard_logging:
                writer.add_scalar("Training_Loss", meanLoss,iteration)
                writer.add_scalar("Training_Accuracy", meanAccuracy, iteration)
            ...
            if iteration == opt.iterationEnd:
                ...
                torch.save(net.state_dict(), '%s/bn_netFinal_%d.pth' % (opt.experiment, epoch+1) )
                break
        ...
        if (epoch+1) % 2 == 0:
            np.save('%s/loss.npy' % opt.experiment, np.array(lossArr ) )
            np.save('%s/accuracy.npy' % opt.experiment, np.array(accuracyArr ) )
            torch.save(net.state_dict(), '%s/bn_net_%d.pth' % (opt.experiment, epoch+1) )
...
if opt.resume == 1:
    ...
    lossArr = checkpoint['loss']
    accuracyArr = checkpoint['accuracy']
    iteration = checkpoint['iteration']
    train(start_epoch, opt.nepoch, iteration, net, optimizer, opt.checkpoint_path)
